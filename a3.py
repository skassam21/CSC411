...
def main(argv=None):  # pylint: disable=unused-argument
    if FLAGS.self_test:
        ...
        # train_data, train_labels = randomly_flip_images(train_data, train_labels)
    ...
    conv1_weights = tf.Variable(
            tf.truncated_normal([5, 5, NUM_CHANNELS, 4],  # 5x5 filter, depth 15.
                                stddev=0.1,
                                seed=SEED, dtype=data_type()))
    conv1_biases = tf.Variable(tf.zeros([4], dtype=data_type()))
    conv2_weights = tf.Variable(tf.truncated_normal(
            [5, 5, 4, 8], stddev=0.1,
            seed=SEED, dtype=data_type()))
    conv2_biases = tf.Variable(tf.constant(0.1, shape=[8], dtype=data_type()))
    fc1_weights = tf.Variable(  # fully connected, depth 512.
            tf.truncated_normal([IMAGE_SIZE // 4 * IMAGE_SIZE // 4 * 8, 512],
                                stddev=0.1,
                                seed=SEED,
                                dtype=data_type()))
    ...
    with tf.Session() as sess:
        ...
        for step in xrange(int(num_epochs * train_size) // BATCH_SIZE):
            ...
            if step % EVAL_FREQUENCY == 0:
                save_path = saver.save(sess, "cnn/model-16-32-{}.ckpt".format(step))
